...
const byRoute = new Map<string, { n: number; ms: number; errors: number }>();
...
export function recordRequest(route: string, status: number, ms: number): void {
  ...
  const entry = byRoute.get(route) || { n: 0, ms: 0, errors: 0 };
  entry.n++;
  entry.ms += ms;
...
function localSnapshot() {
  ...
  return {
    ...
    routes: [...byRoute.entries()]
      .map(([route, v]) => ({ route, n: v.n, avgMs: Math.round(v.ms / v.n), errors: v.errors }))
      .sort((a, b) => b.n - a.n)
      .slice(0, 12),
  };
}
...
export async function collectMetrics(): Promise<any> {
  ...
  const routes = new Map<string, { route: string; n: number; avgMs: number; errors: number }>();
  for (const s of snapshots) {
    for (const r of s.routes) {
      const e = routes.get(r.route) || { route: r.route, n: 0, avgMs: 0, errors: 0 };
  ...
  return {
    ...
    series,
    routes: [...routes.values()].sort((a, b) => b.n - a.n).slice(0, 12),
  };
}
